...
- `get_embedding_model()`: Modeli ilk kullanımda yükler ve bellekte cache'ler.
- `embed_text()`: Tek bir metni embedding'e çevirir.
- `embed_texts()`: Birden fazla metni batch halinde embedding'e çevirir.

Ingestion ve retrieval aynı model instance'ını kullanır. İlk çağrı yavaş olabilir; sonraki çağrılar daha hızlıdır.
...
`clean_answer()` modelin ekleyebildiği `Cevap:`, `Kaynak:` ve `[Parça ...]` gibi istenmeyen etiketleri temizler.
...
Fallback ham chunk döndürdüğü için metin bazen cümlenin ortasından başlayabilir. Bu, ileride geliştirilebilecek bilinen bir sınırlamadır.
...
2. Hata mesajlarını ve çözüm yönlendirmelerini bütün CLI akışında standartlaştırmak.
