...
- Unit testler `56/56` başarılı (chunking, atomik reindex/manifest, indeks güncelliği, interaktif/tek-komut CLI, Rich çıktıları, model yükleme ve LLM cevap temizliği).
...
1. Güvenli `/add` ve onaylı `/remove` dosya yönetimi ekle.
2. Phi modellerini süre ve Türkçe cevap kalitesiyle karşılaştıran benchmark akışı oluştur.
