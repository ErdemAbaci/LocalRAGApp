...
- `app/embeddings.py`: Embedding modelinin lazy-load/cache yönetimi.
...
- Unit testler `16/16` başarılı (chunking, atomik reindex, `/sources` ve `/doctor`).
...
2. Daha açıklayıcı hata mesajları ve çözüm yönlendirmeleri ekle.
3. `rich` ile okunabilir çıktı ve uzun işlemler için durum göstergeleri ekle.
4. `/model` ve `/config` bilgi komutlarını ekle; ilk aşamada ayar değiştirmesinler.
5. Uygulamayı `local-rag`, `local-rag ask`, `local-rag reindex`, `local-rag stats` komutlarıyla çalışacak şekilde paketle.
6. Doküman değişikliklerini algılayıp reindex gerektiğini bildir.
7. Güvenli `/add` ve onaylı `/remove` dosya yönetimi ekle.
8. Phi modellerini süre ve Türkçe cevap kalitesiyle karşılaştıran benchmark akışı oluştur.
