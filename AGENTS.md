...
- `main.py`: CLI döngüsü, komutlar, retrieval kararları, cevap modu ve çıktı gösterimi.
...
- `app/llm.py`: Foundry Local bağlantısı, `phi-4-mini`, cevap temizleme ve kalite doğrulaması.
...
- `tests/test_cli_output.py`: Standart hata gösterimi ve CLI oturum dayanıklılığı testleri.
- `tests/test_embeddings.py`: Yerel embedding snapshot tercihi ve cache-miss fallback testleri.
- `tests/test_llm.py`: Köşeli/parantezli parça etiketlerinin temizlenmesi ve temiz cevap geçerliliği testleri.
...
- Unit testler `29/29` başarılı (chunking, atomik reindex, `/sources`, `/doctor`, Rich çıktıları, embedding yükleme ve LLM cevap temizliği).
...
1. Mevcut reindex/chunking/fallback değişikliklerinin manuel testini tamamla ve kullanıcı isterse commit/push et.
2. `/model` ve `/config` bilgi komutlarını ekle; ilk aşamada ayar değiştirmesinler.
3. Uygulamayı `local-rag`, `local-rag ask`, `local-rag reindex`, `local-rag stats` komutlarıyla çalışacak şekilde paketle.
4. Doküman değişikliklerini algılayıp reindex gerektiğini bildir.
5. Güvenli `/add` ve onaylı `/remove` dosya yönetimi ekle.
6. Phi modellerini süre ve Türkçe cevap kalitesiyle karşılaştıran benchmark akışı oluştur.
