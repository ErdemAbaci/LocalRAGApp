...
- Unit testler `47/47` başarılı (chunking, atomik reindex, interaktif/tek-komut CLI, Rich çıktıları, model yükleme ve LLM cevap temizliği).
...
- Kurulabilir CLI: `local-rag`, `ask`, `reindex`, `stats`, `sources`, `doctor`, `model` ve `config` entrypoint'leri ortak uygulama akışını kullanır.

Yakın hedefleri şu sırayla ele al:

1. Doküman değişikliklerini algılayıp reindex gerektiğini bildir.
2. Güvenli `/add` ve onaylı `/remove` dosya yönetimi ekle.
3. Phi modellerini süre ve Türkçe cevap kalitesiyle karşılaştıran benchmark akışı oluştur.
